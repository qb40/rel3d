'Translucency + Gouraud shading
'Supplement for the vector article
...
DECLARE SUB GouraudTriTrans (ox1%, oy1%, oc1%, ox2%, oy2%, oc2%, ox3%, oy3%, oc3%)
...
TYPE PolyType
        ...
        P2      AS INTEGER
        P3      AS INTEGER
        Basecol AS INTEGER              'Basecolor for translucency
END TYPE
...
SetPolyBaseColor CubePoly()

c$ = INPUT$(1)

'our background image
DIM SHARED Image%(32001)
FOR y = 0 TO 199                'set up nice texture
FOR x = 0 TO 319
    PSET (x, y), y XOR x
NEXT x
NEXT y
GET (0, 0)-(319, 199), Image%   'get it
...
FOR i = 1 TO UBOUND(Poly)
    ...
    IF znormal < 0 THEN
        ...
        Clr1 = (Dot1! * 16) + Poly(i).Basecol       '16 color grad
        Clr2 = (Dot2! * 16) + Poly(i).Basecol
        Clr3 = (Dot3! * 16) + Poly(i).Basecol

        GouraudTriTrans x1, y1, Clr1, x2, y2, Clr2, x3, y3, Clr3
    END IF
...
SUB GouraudTriTrans (ox1%, oy1%, oc1%, ox2%, oy2%, oc2%, ox3%, oy3%, oc3%)
'
'
'             /
'        d1 /  |
...
GourHline:          
    ...
    FOR L% = Gx1% TO Gx2%           'Rasterizer loop
        Tcol% = (Col& \ FIXPOINT)
        TempC% = Tcol% AND 15            '\
        Basecol% = Tcol% - TempC%        ' \   Translucency
        DestCol% = POINT(L%, yy%) AND 15 '  /
        Tcol% = (TempC% + DestCol%) \ 2  ' /
        Tcol% = Tcol% + Basecol%         '/
        PSET (L%, yy%), Tcol%               'Use poke for speed
        Col& = Col& + deltac&               'DDA
    NEXT L%
